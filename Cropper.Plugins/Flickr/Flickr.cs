...
namespace Cropper.SendToFlickr
{
    public class Flickr : IPersistableImageFormat
    {
        ...
        private void menuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                new AuthorizeDialog(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + Settings.CONFIGPATH).Show();
            }
        }
        ...
        private void persistableOutput_ImageCaptured(object sender, ImageCapturedEventArgs e)
        {
            ...
            FileStream stream1 = new FileStream(names1.FullSize, FileMode.CreateNew);
            e.FullSizeImage.Save(stream1, ImageFormat.Png);
            stream1.Close();
            if (e.IsThumbnailed)
            {
                names1 = e.ImageNames;
                stream1 = new FileStream(names1.Thumbnail, FileMode.CreateNew);
                e.ThumbnailImage.Save(stream1, ImageFormat.Png);
                stream1.Close();
            }
            if (this.ConfigExists)
            {
                Settings settings1 = new Settings(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + Settings.CONFIGPATH);
                FlickrNet.Flickr flickr1 = new FlickrNet.Flickr(Settings.APIKEY, Settings.APISHAREDSECRET, settings1.Token);
                PhotoDetails details1 = new PhotoDetails(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + Settings.CONFIGPATH);
        ...
        private bool ConfigExists
        {
            get
            {
                ...
                if (File.Exists(text1 + Settings.CONFIGPATH))
                {
                    return true;
                }
                return false;
