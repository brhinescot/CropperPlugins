...
namespace Cropper.SendToPicasa
{
    public class Plugin : DesignablePluginThatUsesFetchOutputStream, IConfigurablePlugin
    {
        ...
        ///
        /// <remarks>
        ///   Saving the image to a disk file isn't strictly necessary to
        ///   enable upload of the image via the HTML FORM, but it's nice to
        ///   have a cached version of the image in the filesystem.
        /// </remarks>
        protected override void SaveImage(Stream stream, System.Drawing.Image image)
        {
            ...
            {
                Tracing.Trace("+--------------------------------");
                Tracing.Trace("Picasa::{0:X8}::SaveImage ({1})",
        ...
        {
            Tracing.Trace("Picasa::{0:X8}::SaveImageInDesiredFormat", this.GetHashCode());
            if (String.Compare(Extension, "jpg", true) == 0)
            {
        ...
        {
            Tracing.Trace("Picasa::{0:X8}::MenuClick", this.GetHashCode());
        ...
        {
            Tracing.Trace("Picasa::{0:X8}::OptionsSaved", this.GetHashCode());
            Console.ReadLine();
        ...
        public object Settings
        {
            get
            {
                Tracing.Trace("Picasa::{0:X8}::get_Settings", this.GetHashCode());
                return PluginSettings;
            }
            set
            {
                Tracing.Trace("Picasa::{0:X8}::set_Settings", this.GetHashCode());
                PluginSettings = value as PicasaSettings;
            }
        }
